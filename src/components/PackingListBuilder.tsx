...
import { Plus, Search, Trash2, FileDown, Settings2, Box, Package as PackageIcon, Calendar, MapPin, ClipboardList, StickyNote, Edit2, CheckSquare, Square, Scissors, Clipboard, ClipboardCopy, X, ArrowLeftRight, GripVertical, AlertTriangle, Lightbulb, List, CheckCircle, Undo2, Share, Save, User, FileText, AlignLeft } from 'lucide-react';
          ...
          <div className="flex items-center gap-2">
              {activeList?.isCompleted ? (
                  ...
                  <button 
                      ...
                      className="bg-slate-800 hover:bg-amber-600/20 text-slate-400 hover:text-amber-500 border border-slate-700 hover:border-amber-500/50 px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm transition-all hover:scale-105 active:scale-95"
                  >
                      <Undo2 size={18} />
                      Torna a Bozza
                  </button>
              ) : activeList?.isDraftVisible ? (
                  /* STATO 2: Bozza Visibile */
                  <div className="flex gap-2">
                      <button 
                          onClick={handleWithdrawDraft}
                          className="bg-slate-800 hover:bg-rose-950/30 text-slate-500 hover:text-rose-500 border border-slate-700 hover:border-rose-900/50 px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm transition-all hover:scale-105"
                          title="Ritira dal Magazzino"
                      >
                          <X size={18} />
                          Ritira Bozza
                      </button>
                      <button 
                          onClick={handleUpdateDraftVersion}
                          className="bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm transition-all hover:scale-105 active:scale-95"
                      >
                          <Save size={18} />
                          Aggiorna Bozza
                      </button>
                  ...
                  /* STATO 1: Bozza Nascosta */
                  <div className="flex gap-2">
                      <button 
                          onClick={handleShowDraftInWarehouse}
                          className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm transition-all hover:scale-105 active:scale-95"
                      >
                          <Share size={18} />
                          Invia Bozza al Magazzino
                      </button>
                      <button 
                          onClick={handleCompleteList}
                          className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm shadow-lg shadow-blue-900/20 transition-all hover:scale-105 active:scale-95"
            ...
            <div className="px-3 py-2 bg-slate-900 border-b border-slate-800 flex flex-col sm:flex-row items-center gap-3">
                <div className="flex gap-2 w-full items-center">
                    ...
                    <button 
                        ...
                        className="bg-amber-600 hover:bg-amber-500 text-white px-3 py-2 rounded-lg flex items-center gap-2 font-bold text-xs transition-all shrink-0 shadow-lg shadow-amber-900/20"
                    >
                        <Plus size={14} /> Temp.
                    </button>
